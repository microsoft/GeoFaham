...
from autogen_core import AgentRuntime, AgentType, Component, ComponentModel, TypeSubscription
...
from agents.orchestrator_agent.orchestrator import GeoFahamOrchestrator
from agents.orchestrator_agent._prompts import ORCHESTRATOR_FINAL_ANSWER_PROMPT

trace_logger = logging.getLogger(TRACE_LOGGER_NAME)
event_logger = logging.getLogger(EVENT_LOGGER_NAME)


class GeoFahamGroupChatConfig(BaseModel):
    """The declarative configuration for a GeoFahamGroupChat."""
...
class GeoFahamGroupChat(BaseGroupChat, Component[GeoFahamGroupChatConfig]):
    """A team that runs a group chat with participants managed by the GeoFahamOrchestrator.
    ...
    the GeoFahamGroupChat does not support using team as participant.
    ...
    GeoFahamGroupChat with one assistant agent:
        ...
        .. code-block:: python
            ...
            from autogen_agentchat.teams import GeoFahamGroupChat
            ...
            async def main() -> None:
                ...
                team = GeoFahamGroupChat([assistant], model_client=model_client)
    ...
    References:
        ...
        If you use the GeoFahamGroupChat in your work, please cite the following paper:
    ...
    component_config_schema = GeoFahamGroupChatConfig
    component_provider_override = "autogen_agentchat.teams.GeoFahamGroupChat"

    DEFAULT_NAME = "GeoFahamGroupChat"
    ...
    def __init__(
        ...
        model_client2: ChatCompletionClient | None = None,
        *,
        name: str | None = None,
        description: str | None = None,
        ...
        super().__init__(
            ...
            group_chat_manager_name="GeoFahamOrchestrator",
            group_chat_manager_class=GeoFahamOrchestrator,
        ...
        if len(participants) == 0:
            raise ValueError("At least one participant is required for GeoFahamGroupChat.")
        self._model_client = model_client
        if model_client2 is None:
            model_client2 = model_client
        self._model_client2 = model_client2
        self._max_stalls = max_stalls
        self._final_answer_prompt = final_answer_prompt

    async def _init(self, runtime: AgentRuntime) -> None:
        """Initialize the group chat with orchestrator-only visibility.
        
        Unlike the base implementation, participants do NOT subscribe to the group topic,
        so they cannot see messages from other agents. Only the orchestrator sees everything.
        """
        ...
        for participant, agent_type in zip(self._participants, self._participant_topic_types, strict=True):
            ...
            await ChatAgentContainer.register(
                ...
                factory=self._create_participant_factory(
                    self._group_topic_type,  # Still pass group topic for internal messaging
            ...
            # KEY CHANGE: Only subscribe to their own topic, NOT the group topic
            await runtime.add_subscription(
                TypeSubscription(topic_type=agent_type, agent_type=agent_type)
            )
            # REMOVED: Subscription to self._group_topic_type
            # This means agents won't see messages broadcast to the group topic

        # Register the group chat manager (orchestrator)
        ...
        # Orchestrator subscriptions (unchanged - it still sees everything)
        await runtime.add_subscription(
            TypeSubscription(
        ...
        await runtime.add_subscription(
            TypeSubscription(
                topic_type=self._group_topic_type,  # Orchestrator still subscribes to group
                agent_type=group_chat_manager_agent_type.type
            )
        )
        await runtime.add_subscription(
    ...
    ) -> Callable[[], GeoFahamOrchestrator]:
        return lambda: GeoFahamOrchestrator(
            name,
            group_topic_type,
            output_topic_type,
    ...
    def _to_config(self) -> GeoFahamGroupChatConfig:
        participants = [participant.dump_component() for participant in self._participants]
        termination_condition = self._termination_condition.dump_component() if self._termination_condition else None
        return GeoFahamGroupChatConfig(
            name=self.name,
    ...
    @classmethod
    def _from_config(cls, config: GeoFahamGroupChatConfig) -> Self:
