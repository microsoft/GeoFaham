...
"""
Raster routes - upload and info endpoints.
"""
import json
import logging
import os
...
from backend.routes.config import USER_ARTIFACTS_JSON, USER_DATA_PATH
...
async def upload_raster(
    ...
    try:
        ...
        if not file.filename.lower().endswith((".tif", ".tiff", ".geotiff")):
            raise HTTPException(
                status_code=400, detail="File must be a GeoTIFF (.tif, .tiff)"
            )
        ...
        if not is_valid:
            os.remove(file_path)
            raise HTTPException(
                status_code=400, detail=f"Invalid raster file: {error_msg}"
            )
